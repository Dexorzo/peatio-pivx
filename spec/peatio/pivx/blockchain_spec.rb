RSpec.describe Peatio::Pivx::Blockchain do
  context :features do
    it 'defaults' do
      blockchain1 = Peatio::Pivx::Blockchain.new
      expect(blockchain1.features).to eq Peatio::Pivx::Blockchain::DEFAULT_FEATURES
    end

    it 'override defaults' do
      blockchain2 = Peatio::Pivx::Blockchain.new(cash_addr_format: true)
      expect(blockchain2.features[:cash_addr_format]).to be_truthy
    end

    it 'custom feautures' do
      blockchain3 = Peatio::Pivx::Blockchain.new(custom_feature: :custom)
      expect(blockchain3.features.keys).to contain_exactly(*Peatio::Pivx::Blockchain::SUPPORTED_FEATURES)
    end
  end

  context :configure do
    let(:blockchain) { Peatio::Pivx::Blockchain.new }
    ...
    it 'currencies and server configuration' do
      currencies = [{ id: :pivx,
                      ...
                      options: {} }]
      settings = { server: 'http://admin:changeme@127.0.0.1:51475',
  ...
  context :latest_block_number do
    ...
    let(:server) { 'http://admin:changeme@127.0.0.1:51475' }
    let(:server_without_authority) { 'http://127.0.0.1:51475' }
    ...
    let(:blockchain) do
      Peatio::Pivx::Blockchain.new.tap {|b| b.configure(server: server)}
    end
    ...
    it 'raises error if there is error in response body' do
      stub_request(:post, 'http://127.0.0.1:51475')
        .with(body: { jsonrpc: '1.0',
                      method: :getblockcount,
                      params:  [] }.to_json)
  ...
  context :build_transaction do
    ...
    context 'three vout tx' do
      ...
      let(:blockchain) do
        Peatio::Pivx::Blockchain.new.tap { |b| b.configure(currencies: [currency]) }
      end
    ...
    context 'multiple currencies' do
      ...
      let(:blockchain) do
        Peatio::Pivx::Blockchain.new.tap do |b|
  ...
  context :fetch_block! do
    ...
    let(:getrawtransaction_response_file_bda40f63c61d76463e5c5ca8750c8f711d7f8fc0735b1c629e06fc3e76ce2c4e) do
      File.join('spec', 'resources', 'getrawtransaction_bda40f63c61d76463e5c5ca8750c8f711d7f8fc0735b1c629e06fc3e76ce2c4e', 'bda40f63c61d76463e5c5ca8750c8f711d7f8fc0735b1c629e06fc3e76ce2c4e.json')
      end

    let(:getrawtransaction_response_file_7a43f3b8d95c475245859753ee3bd821309a324b377d4a6e9a54f4c073c32ae1) do
      File.join('spec', 'resources', 'getrawtransaction_7a43f3b8d95c475245859753ee3bd821309a324b377d4a6e9a54f4c073c32ae1', '7a43f3b8d95c475245859753ee3bd821309a324b377d4a6e9a54f4c073c32ae1.json')
    end
    ...
    let(:getrawtransaction_response_bda40f63c61d76463e5c5ca8750c8f711d7f8fc0735b1c629e06fc3e76ce2c4e) do
      getrawtransaction_response_file_bda40f63c61d76463e5c5ca8750c8f711d7f8fc0735b1c629e06fc3e76ce2c4e
        .yield_self { |file_path| File.open(file_path) }
        .yield_self { |file| JSON.load(file) }
      end

    let(:getrawtransaction_response_7a43f3b8d95c475245859753ee3bd821309a324b377d4a6e9a54f4c073c32ae1) do
      getrawtransaction_response_file_7a43f3b8d95c475245859753ee3bd821309a324b377d4a6e9a54f4c073c32ae1
        .yield_self { |file_path| File.open(file_path) }
        .yield_self { |file| JSON.load(file) }
    end
    ...
    before do
      ...
      stub_request(:post, server_without_authority)
        .with(body: { jsonrpc: '1.0',
                      method: :getblock,
                      params:  ['be8e7df63dc77990c21b469effc73d4a7d96a989a2eeaa26ef6f5f6ba9eab59d',true] }.to_json)
        .to_return(body: getblock_response.to_json)

      stub_request(:post, server_without_authority)
        .with(body: { jsonrpc: '1.0',
                      method: :getrawtransaction,
                      params:  ['bda40f63c61d76463e5c5ca8750c8f711d7f8fc0735b1c629e06fc3e76ce2c4e',1] }.to_json)
        .to_return(body: getrawtransaction_response_7a43f3b8d95c475245859753ee3bd821309a324b377d4a6e9a54f4c073c32ae1.to_json)
    end
    ...
    let(:server_without_authority) { 'http://127.0.0.1:51475' }
    let(:blockchain) do
      Peatio::Pivx::Blockchain.new.tap { |b| b.configure(server: server, currencies: [currency]) }
    end
  ...
  context :load_balance_of_address! do
    ...
    let(:response_file) do
      File.join('spec', 'resources', 'listaddressgroupings', 'bda40f63c61d76463e5c5ca8750c8f711d7f8fc0735b1c629e06fc3e76ce2c4e.json')
    end
    ...
    context 'address with balance is defined' do
      it 'requests rpc listaddressgroupings and finds address balance' do
        address = 'MU88ekFEL8hDg4GJoTB3cAJBaL9LFyquyB'

        result = blockchain.load_balance_of_address!(address, :pivx)
        expect(result).to be_a(BigDecimal)
        expect(result).to eq('0.10000000'.to_d)
      end

      it 'requests rpc listaddressgroupings and finds address with zero balance' do
        address = 'MHaHCEmSaXwPEZUdAWb6bkcfzEvSqs85C9'
    ...
    context 'address is not defined' do
      it 'requests rpc listaddressgroupings and do not find address' do
        ...
        expect{ blockchain.load_balance_of_address!(address, :pivx)}.to raise_error(Peatio::Blockchain::UnavailableAddressBalanceError)
      end
    end
    ...
    context 'client error is raised' do
      ...
      it 'raise wrapped client error' do
        expect{ blockchain.load_balance_of_address!('anything', :pivx)}.to raise_error(Peatio::Blockchain::ClientError)
      end
    end
  end
end
